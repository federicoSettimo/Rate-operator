...
VectorXd roqj::run_single_iterations (bool verbose) const {
  ...
  // Time evolution
  /*
    Note: the whole time evolution can be made more efficient by estimating the waiting time distribution
    and diagonalizing R only when a jump happens. Now, the eigenvalues are calculated at each timestep
  */
  for (double t = _t_i; t <= _t_f; t += _dt) {
    ...
    for (int i = 0; i < _N_states; ++i) {
      ...
      psi[i] *= exp(-I*arg(psi[i](0))); // WLOG psi[i](0) is real (just a global phase)
      psi[i] = psi[i].normalized();
    }
